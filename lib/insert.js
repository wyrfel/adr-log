...
module.exports.insert = function(str, options) {
  ...
  var open = typeof options.open === 'string' ? options.open : '<!-- toc -->' + os.EOL + os.EOL;
  var close = typeof options.close === 'string' ? options.close : '<!-- tocstop -->';
  var obj;

  var newlines = '';
  var m = new RegExp(`${os.EOL}+$`).exec(str);
  if (m) newlines = m[0];
  ...
  var resultString = sections.join(os.EOL + os.EOL) + newlines;
...
module.exports.insertAdrToc = function(str, tocContent, options, dirinsertAdr) {
  ...
  if (sections.length === 2) {
    sections.splice(1, 0, open + toc(tocContent, options).content + os.EOL + os.EOL + close);
  }
